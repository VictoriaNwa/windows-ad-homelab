...
6. Go to DC's Settings → Network
   - **Adapter 1:** "Enable" -> Internal Network
...
5. Right click Start Menu -> "Network Connections" -> "Change adapter options" -> Right click adapter -> 
     - Rename "Internal Network"
     - Right click again -> "Properties" -> "Internet Protocol Version 4 (TCP/IPv4)
     - Use the following IP address: 192.168.0.1
     - Subnet mask : 255.255.255.0
     - No default gateway
     - Use the following DNS Server address: put the same as IP or 127.0.0.1 (127.0.0.1 addresses back to self, so both essentially do the same thing) -> OK
...
## 4. Installing DHCP and Setting up Scope
